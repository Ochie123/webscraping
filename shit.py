...
db.query(create_pages_table + ';' + create_links_table, fetchall=False)
db.query(create_links_table, fetchall=False)
...
def store_page(url):
    try:
        db.query('''
            INSERT INTO pages (url, created_at)
            VALUES (:url, CURRENT_TIMESTAMP)
        ''', url=url)
    except IntegrityError as ie:
        # This page already exists, pass
        pass

def store_link(url, url_to):
    try:
        db.query('''
            INSERT INTO links (url, url_to)
            VALUES (:url, :url_to)
        ''', url=url, url_to=url_to)
    except IntegrityError as ie:
        # This link already exists, pass
        pass
